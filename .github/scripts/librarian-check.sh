...
check_conventions_sync() {
  local fail=0 conventions_changed docs_changed docs_dir_changed
  local adr_count commands_changed spec_updated conventions_or_hooks
  local contributing_updated changelog_changed
  ...
  if [ -n "$docs_dir_changed" ]; then
    ...
    [ "$adr_count" -eq 0 ] && echo "FAIL: docs/ changed but docs/adr/ has no ADRs" && fail=1
  fi
  ...
  [ -n "$changelog_changed" ] && echo "WARN: CHANGELOG.md modified manually — it is managed by semantic-release"
