...
- `run.ts` — Invoke a member or core agent as an LLM agent (provider override, `--detect`; per-request provider timeout via `failover.requestTimeoutMs`, default 60s)
...
3. **`.githooks/pre-push`** — Blocks pushes to main and requires `type/issue-NUMBER-...` branch names (`.githooks/prepare-commit-msg` installs the commit template)
4. **`.github/workflows/pr.yml`** — CI re-validates all PR standards (commit messages, tests, file sizes, docs, member structure, secrets)

Banned commit subjects include: `wip`, `fix stuff`, `update`, `changes`, `misc`, `temp`, `asdf`, `cleanup`, `test123`. The custom rule lives in `docs/conventions/commitlint.config.ts` and is tested in `tests/commitlint.test.ts`.
...
- **`agenthood run` shuts down gracefully** — first `SIGINT`/`SIGTERM` stops at the next step boundary, flushing the partial trace and exiting `130`; a second signal or a 5s watchdog forces exit
