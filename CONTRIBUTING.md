...
When a command wraps a caught error in a friendlier message, the original is attached via `{ cause }` so the underlying failure stays in the stack output (`@eslint/js` v10 `preserve-caught-error` enforces this across `src/`).
