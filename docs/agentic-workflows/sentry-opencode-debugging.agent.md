...
5. **Systemic? escalate to Seer** — if the issue recurs across releases or the root cause
   is still unclear after Step 4, run `begin_seer_issue_fix` and record the plan link
   - Single-issue fixes stay in-protocol — no Seer
