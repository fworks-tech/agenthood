  ...
  async complete(request: LLMRequest): Promise<LLMResponse> {
    try {
      const response = await this.client.chat.completions.create({
        model: this.model,
        messages: request.messages as unknown as Groq.Chat.Completions.ChatCompletionMessageParam[],
        tools: request.tools as unknown as Groq.Chat.Completions.ChatCompletionTool[],
