...
const inputSchema: JSONSchema = {
  type: 'object',
  properties: {
    role: { type: 'string', description: 'Agent role to delegate to (e.g. developer, reviewer)' },
...
export class SubagentTaskSkill implements ISkill {
  name = 'task'
  description = 'Delegate a task to a specialized subagent. Use this for work that needs focused expertise or isolated context (e.g. code review, architecture planning, debugging). Returns the subagent output.'
  ...
  async execute(input: unknown, context: ExecutionContext): Promise<SkillResult> {
    ...
    try {
      ...
      return { success: true, output: result.output, artifacts: result.artifacts }
    } catch (err) {
      if (err instanceof AgentNotFoundError) {
        return { success: false, output: '', error: `No agent found for role "${role}". Available: ${this.agentRegistry.list().join(', ')}` }
      }
